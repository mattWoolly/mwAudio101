...
//
// core/calibration/ModulatorModuleConstants.h — the (PI) DESIGN-UNIT layout constants
// for ui/modules/ModulatorModule (task 117). The module lays its children out in
// design units only (no pixel math) [docs/design/10-ui.md §5.3]; the parent passes a
// design-space rectangle to layoutDesignUnits() and the module sub-divides it with
// the proportions named here.
//
...
// The number of control cells the module lays out in its single row:
// LFO Rate, LFO Shape, LFO->Pitch depth, LFO->PWM depth, LFO->Cutoff depth.
inline constexpr int kControlCellCount = 5;
